...
public struct SpecSecurityScheme: Codable, Equatable, Changeable {
    ...
    public var type: SpecSecuritySchemeType
    ...
    public init(from decoder: Decoder) throws {
        ...
        case .apiKey:
            type = .apiKey(
                name: try container.decode(forKey: .apiKeyName),
                in: try container.decode(forKey: .apiKeyLocation)
            )

        case .oauth2:
            type = .oauth2(flows: try container.decode(forKey: .oauthFlows))

        case .http:
            let scheme: SpecSecurityHTTPScheme
            ...
            case CodingHTTPScheme.bearer.rawValue:
                scheme = .bearer(format: try container.decodeIfPresent(forKey: .httpSchemeBearerFormat))
        ...
        case .openIDConnect:
            type = .openIDConnect(url: try container.decode(forKey: .openIDConnectURL))
        }
