...
class SwaggerOAuthAuthorizationCodeFlowSpec: QuickSpec {

    // MARK: - Instance Methods

    override func spec() {
        var flow: SwaggerOAuthAuthorizationCodeFlow!
        var flowYAML: String!

        describe("Flow without any scopes") {
            beforeEach {
                flow = SwaggerOAuthAuthorizationCodeFlowSeeds.noScopes
                flowYAML = SwaggerOAuthAuthorizationCodeFlowSeeds.noScopesYAML
            }

            it("should be correctly decoded from YAML string") {
                do {
                    let decodedFlow = try YAMLDecoder.test.decode(
                        SwaggerOAuthAuthorizationCodeFlow.self,
                        from: flowYAML
                    )
        ...
        describe("Flow with scopes for reading and writing app information") {
            beforeEach {
                flow = SwaggerOAuthAuthorizationCodeFlowSeeds.readWriteApps
                flowYAML = SwaggerOAuthAuthorizationCodeFlowSeeds.readWriteAppsYAML
            }
